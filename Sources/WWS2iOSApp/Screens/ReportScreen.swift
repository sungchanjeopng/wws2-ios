                    ...
                    ForEach(devices, id: \.id) { device in
                        HStack(spacing: 14) {
                            ...
                            VStack(alignment: .leading, spacing: 2) {
                                ...
                                Text("Interface Meter").font(.system(size: 12)).foregroundStyle(AppColors.weakText)
                            }
                            Spacer()
        ...
        VStack(spacing: 0) {
            ScrollView {
                VStack(alignment: .leading, spacing: 0) {
                    ...
                    SectionLabel(title: "Settings", accent: reportPurple)
                    Spacer().frame(height: 10)
                    SettingsCard(rows: [
                    ...
                    SectionLabel(title: "Waveform", accent: lightBlue)
                    Spacer().frame(height: 10)
            ...
            HStack(spacing: 12) {
                ...
                Button(action: onShare) {
                    Text("Export HTML").font(.system(size: 16, weight: .heavy)).foregroundStyle(.white)
                        .frame(maxWidth: .infinity).frame(height: 52)
...
private struct HeaderCard: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            Spacer().frame(height: 10)
            Text("Interface Meter  ·  FW \(data.firmwareVersion.isEmpty ? "—" : data.firmwareVersion)")
...
private struct WaveBlock: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            InterfaceEchoChart(echo: reading).frame(height: 190)
        }
    }
}
